...
function Messages() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await fetch(
        `/custom/api/messages/get_inbox.php?view=${view}`,
        {
          credentials: 'include'
        }
      );
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      // Get recipient info from the thread
      const firstMessage = threadMessages[0];
      const isOriginalSender = firstMessage.senderType === 'staff';
      const recipientType = isOriginalSender ? firstMessage.recipientType : firstMessage.senderType;
      const recipientId = isOriginalSender
        ? (firstMessage.recipientType === 'client' ? firstMessage.recipientClientId : firstMessage.recipientUserId)
        : (firstMessage.senderType === 'client' ? firstMessage.senderClientId : firstMessage.senderUserId);
                  ...
                  >
                    <div className="flex items-start justify-between mb-1">
                      <span className={`font-medium ${!msg.isRead ? 'text-gray-900 font-bold' : 'text-gray-700'}`}>
                        {view === 'inbox' ? msg.senderName : msg.recipientName}
                      </span>
                      {!msg.isRead && (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {threadMessages.map((msg, index) => {
                    const isCurrentUser = msg.senderType === 'staff'; // Simplified for now
                    return (
                      <div
                        key={msg.id}
